import { ISvgPoint, IPoint } from '../interfaces';
import { round } from '@sketchmine/sketch-file-format';
...
export class CurvePoint {
  ...
  protected isMoveTo(point: ISvgPoint): boolean {
    return point && point.code === 'M' || point && point.code === 'm';
  }
  protected isLineTo(point: ISvgPoint): boolean {
    return point && point.code === 'L' || point && point.code === 'l';
  }
  protected isCurveTo(point: ISvgPoint): boolean {
    return point && point.code === 'C' || point && point.code === 'c';
  }
  protected isArc(point: ISvgPoint): boolean {
    return point && point.code === 'A' || point && point.code === 'a';
  }
  protected isSmoothCurveTo(point: ISvgPoint): boolean {
    return point && point.code === 'S' || point && point.code === 's';
  }
  protected isQuadratic(point: ISvgPoint): boolean {
    return point && point.code === 'Q' || point && point.code === 'q';
  }
