...
export function updateNgModuleDecoratorProperties(
  ...
  const decorators = findNodes(sourceFile, ts.SyntaxKind.Decorator) as ts.Decorator[];
  ...
  decorators.forEach((decorator: ts.Decorator) => {
    ...
    if (
      ...
      decorator.expression.arguments.forEach((argument: ts.ObjectLiteralExpression) => {

        const prop = argument.properties.find((prop: ts.PropertyAssignment) => {
        ...
        if (prop) {
          ...
          if (ts.isArrayLiteralExpression(prop.initializer)) {
            ...
            console.warn(`Property Type <${ts.SyntaxKind[prop.initializer.kind]}> is currently not implementet!`);
          }
